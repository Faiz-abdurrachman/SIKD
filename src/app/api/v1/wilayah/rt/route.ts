...
import { createRTSchema } from "@/validations/wilayah.schema";
...
export async function GET() {
  try {
    ...
    return successResponse(data.rt);
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    const parsedBody = createRTSchema.safeParse(body);
    ...
    const data = await wilayahService.createRT(parsedBody.data, session.user.id, getRequestMeta(request));
