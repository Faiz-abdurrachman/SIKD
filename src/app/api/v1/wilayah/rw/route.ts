...
import { createRWSchema } from "@/validations/wilayah.schema";
...
export async function GET() {
  try {
    ...
    return successResponse(data.rw);
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    const parsedBody = createRWSchema.safeParse(body);
    ...
    const data = await wilayahService.createRW(parsedBody.data, session.user.id, getRequestMeta(request));
