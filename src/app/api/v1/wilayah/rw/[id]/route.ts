...
import { updateRWSchema } from "@/validations/wilayah.schema";
...
export async function PUT(request: Request, context: RouteContext) {
  try {
    ...
    const parsedBody = updateRWSchema.safeParse(body);
    ...
    const data = await wilayahService.updateRW(id, parsedBody.data, session.user.id, getRequestMeta(request));
...
export async function DELETE(request: Request, context: RouteContext) {
  try {
    ...
    const { id } = await context.params;
    await wilayahService.deleteRW(id, session.user.id, getRequestMeta(request));
