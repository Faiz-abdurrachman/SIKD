...
export async function GET(request: Request) {
  const profiler = createRequestProfiler(request, "GET /api/v1/mutasi");
...
export async function POST(request: Request) {
  const profiler = createRequestProfiler(request, "POST /api/v1/mutasi");
  ...
  try {
    ...
    if (!canAccess(session.user.role, "mutasi", "create")) {
      return profiler.finish(errorResponse(ERROR_CODES.FORBIDDEN, "Akses ditolak", 403), {
        result: "forbidden",
      });
    }
    ...
    const parsedBody = createMutasiSchema.safeParse(body);
    profiler.mark("validation");
    ...
    const data = await (async () => {
      try {
        return await mutasiService.create(parsedBody.data, session.user.id, getRequestMeta(request));
      } finally {
        profiler.mark("service");
      }
    })();
