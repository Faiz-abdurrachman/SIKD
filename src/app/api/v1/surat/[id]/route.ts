...
import { updateSuratSchema } from "@/validations/surat.schema";
...
export async function PUT(request: Request, context: RouteContext) {
  try {
    ...
    if (!canAccess(session.user.role, "surat", "update")) {
      return errorResponse(ERROR_CODES.FORBIDDEN, "Akses ditolak", 403);
    }

    const body = await request.json();
    const parsedBody = updateSuratSchema.safeParse(body);
    ...
    const { id } = await context.params;
    const data = await suratService.update(id, parsedBody.data, session.user.id, getRequestMeta(request));
...
export async function DELETE(request: Request, context: RouteContext) {
  try {
    ...
    if (!canAccess(session.user.role, "surat", "delete")) {
      return errorResponse(ERROR_CODES.FORBIDDEN, "Akses ditolak", 403);
    }

    const { id } = await context.params;
    const data = await suratService.delete(id, session.user.id, getRequestMeta(request));
